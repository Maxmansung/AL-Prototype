...
class profileModel extends profile
{
    ...
    public static function insertProfile($profileController, $type){
        $db = db_conx::getInstance();
        if ($type == "Insert") {
            $req = $db->prepare("INSERT INTO Profile (id, password, profilepicture, email, login, ip, AccountType, gamestatus, avatar, achievements, bio, country, gender, age, playStatistics, uploadSecurity, passwordRecovery, passwordRecoveryTimer, cookieKey) VALUES (:profileID, :password2, :profilePicture, :email, now(), :loginIP, :accountType, :gameStatus, :avatar, :achievements, :bio, :country, :gender, :age, :playStatistics, :uploadSecurity, :passwordRecovery, :passwordRecoveryTimer, :cookieKey)");
        } elseif ($type == "Update"){
            $req = $db->prepare("UPDATE Profile SET password= :password2, profilepicture= :profilePicture, email= :email, login= now(), ip= :loginIP, AccountType= :accountType, gamestatus= :gameStatus, avatar= :avatar, achievements= :achievements, bio= :bio, country= :country, gender= :gender, age= :age, playStatistics= :playStatistics, uploadSecurity= :uploadSecurity, passwordRecovery= :passwordRecovery, passwordRecoveryTimer= :passwordRecoveryTimer, cookieKey= :cookieKey WHERE id= :profileID");
        }
