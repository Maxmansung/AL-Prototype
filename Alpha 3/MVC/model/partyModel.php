...
class partyModel extends party
{
    ...
    public static function insertParty($controller, $type){
        $db = db_conx::getInstance();
        if ($type == "Insert") {
            $req = $db->prepare("INSERT INTO Party (partyID, mapID, members, partyName, pendingRequests, pendingBans, playersKnown, overallZoneExploration) VALUES (:partyID, :mapID, :members, :partyName, :pendingRequests, :pendingBans, :playersKnown, :overallZoneExploration)");
        } elseif ($type == "Update") {
            $req = $db->prepare("UPDATE Party SET mapID= :mapID, members= :members, partyName= :partyName, pendingRequests= :pendingRequests, pendingBans= :pendingBans, playersKnown= :playersKnown, overallZoneExploration= :overallZoneExploration WHERE partyID= :partyID");;
        }
