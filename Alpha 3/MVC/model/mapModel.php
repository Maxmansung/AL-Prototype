...
class mapModel extends map
{
    private function __construct($mapmodel)
    {
        ...
        $this->groups = json_decode($mapmodel['groups']);
        $this->zones = json_decode($mapmodel['zones']);
    ...
    public static function insertMap($mapcontroller, $type){
        $db = db_conx::getInstance();
        if ($type == "Insert") {
            $req = $db->prepare("INSERT INTO Map (mapID, name, maxPlayerCount, edgeSize, maxPlayerStamina, maxPlayerInventorySlots, avatars, groups, zones, currentDay, dayDuration, baseNightTemperature, baseSurvivableTemperature, basePlayerTemperatureModifier, dayEndTime, temperatureRecord, gameType) VALUES (:mapID, :name2, :maxPlayerCount, :edgeSize, :maxPlayerStamina, :maxPlayerInventorySlots, :avatars, :groups, :zones, :currentDay, :dayDuration, :baseNightTemperature, :baseSurvivableTemperature, :basePlayerTemperatureModifier, :dayEndTime, :temperatureRecord, :gameType)");
        } elseif ($type == "Update"){
            $req = $db->prepare("UPDATE Map SET name= :name2, maxPlayerCount= :maxPlayerCount, edgeSize= :edgeSize, maxPlayerStamina= :maxPlayerStamina, maxPlayerInventorySlots= :maxPlayerInventorySlots, avatars= :avatars, groups= :groups, zones= :zones, currentDay= :currentDay, dayDuration= :dayDuration, baseNightTemperature= :baseNightTemperature, baseSurvivableTemperature= :baseSurvivableTemperature, basePlayerTemperatureModifier= :basePlayerTemperatureModifier, dayEndTime= :dayEndTime, temperatureRecord= :temperatureRecord, gameType= :gameType WHERE mapID= :mapID");
        }
        ...
        $req->bindParam(':groups', json_encode($mapcontroller->getGroup()));
        $req->bindParam(':zones', json_encode($mapcontroller->getZone()));
