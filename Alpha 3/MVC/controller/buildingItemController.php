...
class buildingItemController
{
    ...
    public static function storageItemTransfer($itemIDpass, $avatarID)
    {
        ...
        if (in_array($itemID, $avatar->getInventory()) === true) {
        ...
        } else if (in_array($itemID, $storage->getItems())) {
        ...
        foreach ($itemsUsed as $material) {
            for ($x = 0; $x < $material["materialNeeded"]; $x++) {
                if ($zone->getStorage() == true) {
                    $zoneItems = itemModel::getItemArray($storage->getItems());
                } else {
                    $zoneItems = itemModel::getItemArray($zone->getItems());
                }
                $removed = false;
                foreach ($zoneItems as $item) {
                    if ($removed == false) {
                        if ($material["materialRequired"] == $item["itemTemplateID"]) {
                            if ($zone->getStorage() == true) {
                                $storage->removeItem($item["itemID"]);
                            } else {
                                $zone->removeItem($item["itemID"]);
                            }
    ...
    {
        $avatar = new avatarController($avatarID);
        if (in_array($itemID, $avatar->getInventory()) === false) {
            return array("ERROR" => 3);
        } else {
            $item = new itemController($itemID);
    ...
    public static function upgradeStorage($avatarID)
    {
        ...
        foreach ($upgradeItems as $requirement => $number) {
            $counter = 1;
            $allItemsExist = false;
            $itemList = itemController::getItemArray($storage->getItems());
    ...
    public static function checkItems($avatarID)
    {
        ...
        if ($zone->getStorage() == true) {
            $storage = new storageController("", $zone->getZoneID());
            $zoneItems = itemController::getItemArray($storage->getItems());
        } else {
            $zoneItems = itemController::getItemArray($zone->getItems());
        }
    ...
    public static function returnZoneStorage($avatarID)
    {
        ...
        $backpack = itemController::getItemArray($avatar->getInventory());
        if ($storage->getStorageID() == "") {
            $storageItems = array("ERROR" => 7);
        } else {
            $storageItems = itemController::getItemArray($storage->getItems());
        }
