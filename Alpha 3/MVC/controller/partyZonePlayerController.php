...
///////THIS CONTROLLER IS USED FOR THE "PLAYERS" PAGE TO SHOW THE PLAYERS WITHIN THE GAME//////
...
class partyZonePlayerController
{
    ...
    private function __construct($avatarModel,$groupModel,$alive,$known,$ingroup,$isPlayer,$inZone,$inviting,$kicking,$votingChoice,$awaitingInvite,$pendingRequest)
    {
    ...
    public static function kickPlayer($avatarID,$playerID){
        ...
        if (in_array($playerID,$party->getMembers()) != true){
            ...
            $party->uploadParty();
            chatlogGroupController::playerKicking($avatarID,$party->getPartyID());
            return array("SUCCESS"=>true);
        }
    }
    ...
    public static function getPlayerDetails($avatarID)
    {
        ...
        foreach ($avatarList as $avatarSingle) {
            ...
            $tempObject = new partyZonePlayerController($avatarModel, $groupModel, $alive, $known, $ingroup, $isPlayer, $inZone, $inviting, $kicking, $votingVision, $awaitingInvite, $pendingRequest);
