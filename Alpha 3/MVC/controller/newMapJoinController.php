...
class newMapJoinController
{
    ...
    public static function createNewMap($name,$player,$invent,$edge,$stamina,$length,$temp,$stemp,$tempm,$type)
    {
        ...
        for ($y = 0; $y < $edge; $y++) {
            for ($x = 0; $x < $edge; $x++) {
                ...
                $finalZoneID .= $counter;
                $zone->newZone($map->getMapID(), $finalZoneID, $x, $y,$forestLoc);
                $zone->postZone();
                $counter += 1;
            }
        }
    ...
    public static function addAvatar($mapID,$profileID)
    {
        ...
        if (count($mapcheck->getAvatars()) == 0){
            $mapcheck->setDayEndTime(mapController::createTimeStamp($mapcheck->getDayDuration()));
        }
        ...
        foreach ($startingItems as $item) {
            ...
            $object->createNewItemByID($item, $mapcheck->getMapID());
            $object->insertItem();
        ...
        if ($checkAvatar->getAvatarID() == ""){
            return array("ERROR"=>"The avatar was not created successfully");
        } else {
    ...
    public static function duplicateMap($mapController){
        ...
        elseif ($mapController->getGameType() == "Tutorial") {
            if (strpos($mapController->getName()," again") > 0){
                $nameTemp = $mapController->getName();
                $name = str_replace(" again","",$nameTemp);
            } else {
                $name = $mapController->getName();
                $name .=" again";
            }
            newMapJoinController::createNewMap($name, $mapController->getMaxPlayerCount(), $mapController->getMaxPlayerInventorySlots(), $mapController->getEdgeSize(), $mapController->getMaxPlayerStamina(), $mapController->getDayDuration(), 1, $mapController->getBaseSurvivableTemperature(), $mapController->getBaseAvatarTemperatureModifier(),$mapController->getGameType());
        }
    }
}