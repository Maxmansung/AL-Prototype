...
function inGameList(response){
    ...
    for (var player in response) {
        ...
        if (response[player].avatarAlive === true){
            ...
            $("#playerdiv" + player).append("<div class='playerdivname'><strong><strike><a class='usernamelink' href='/user.php?u=" + response[player].avatarName + "'>" + response[player].avatarName + "</a></strike></strong></div>")
                    ...
                    if (players[inside].inParty === true) {
                        switch (players[inside].votingChoice[votingOn]) {
                            ...
                            case 2:
                                ...
                                break;
                            default:
                                console.log("ERROR with agree invite variable for player " + players[inside].avatarName + ". Array = " + players[inside].votingChoice[votingOn]);
                                break;
                        }
                    }
                }
