...
function buildGamePageHUD(data){
    ...
    createStaminaColour(data.staminaCurrent,data.staminaMax);
    createThermometer(data.nightTemp,data.playerTemp);
    createStatusArray(data.playerStatus);
    createSurvivalChance(data.outcome);
}
...
function createStaminaColour(current,max){
    var colours = "rgba(255,255,0,0.8)0%,";
    var percentage = Math.round((current/max)*100);
    if (percentage < 5){
        percentage = 5;
    }
    if (percentage > 99){
        colours = "rgba(255,255,0,1)0%,rgba(255,255,0,1)100%"
    } else {
        colours += "rgba(255,255,0,1)" + (percentage - 5) + "%,rgba(255,255,255,1)" + (percentage) + "%,";
        colours += "rgba(255,255,255,1)100%";
    }
    $(".HUDstaminaBar").css("background","linear-gradient(to right, "+colours+")");
}

function createThermometer(temp,current) {
    var difference = parseInt(temp)-parseInt(current);
    var padding = "0 0 0 0";
    ...
    $(".temperatureSurvivalArrow").css("padding",padding);
    $(".thermometerTemp1").empty().append(temp*-1);
    $(".thermometerTemp0").empty().append((temp-10)*-1);
    $(".thermometerTemp2").empty().append((temp+10)*-1);
}

function createSurvivalChance(outcome){
    var colour = "100,255,100,0.8";
    var colour2 = "";
    var text = "At least you'll make it through tonight...";
    switch (outcome){
        case "dead":
            colour = "255,0,0,0.8";
            text = "You are already dead, you shouldn't see this!!";
            break;
        case "starve":
            colour = "154, 3, 30,0.8";
            text = "You'll starve to death if you don't eat before nightfall";
            break;
        case "freeze":
            colour = "7, 160, 195,0.8";
            text = "It's too cold outside, get warmer or you'll freeze tonight";
            break;
        case "injured":
            colour = "255,125,0,0.8";
            text = "It's too cold outside, get warmer or you'll get frostbite tonight";
            break;
        case "burnt":
            colour = "255,200,0,0.8";
            text = "Sleeping by an active lava flow seems like a good way to get yourself killed...";
        default:
            break;
    }
    $(".HUDSurvivalChances").css({"-moz-box-shadow":"0 0 5px 3px rgba("+colour+")","-webkit-box-shadow":"0 0 5px 3px rgba("+colour+")","box-shadow":"0 0 5px 3px rgba("+colour+")","background":"rgba("+colour+")"});
    $(".HUDSurvivalText").empty().append(text);
}
...
function createActionButtons(actions){
    ...
    for (var x in actions){
        ...
        switch (actions[x]){
            case 1:
                actionButtonVar = new actionButton("fas fa-blind","Search","Spend 2 stamina to search the zone for items","searchZone()");
                break;
            case 2:
...
function createMapZoneDetails(data) {
    $(".zoneInformationBox").empty().append('<div class="pt-2 font-weight-bold" align="center">['+data.xAxis+'/'+data.yAxis+']</div>');
    if (data.known === false){
        $(".zoneInformationBox").append('<div align="center" class="pt-2 grayColour font-size-2x">Unknown Zone</div>');
    } else {
        var depleted = "";
        if (data.depleted === 0){
            depleted = "Depleted"
    ...
    $(".zoneInformationBox").append('<div class="row p-2 align-items-center">' +
        '<div class="input-group py-2">' +
        '<select class="custom-select font-weight-bold" id="zoneMarkerSelector" value="'+data.marker+'">' +
