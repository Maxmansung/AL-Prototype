...
class profileController extends profile
{
    ...
    public static function activateConfirm($username,$password)
    {
        ...
        if ($mapID != 0) {
            ...
            if (array_key_exists("ERROR", $response)) {
                ...
                return array("ALERT" => 23, "DATA" => $profile->getProfileName());
            }
        } else {
            $profile->uploadProfile();
    ...
    public static function updatePassword($newPassword,$security,$username){
        $usernameClean = preg_replace(data::$cleanPatterns['textSpace'],"",$username);
        $profile = new profileController($usernameClean);
        if ($profile->getProfileID() != $username){
            return array("ERROR"=>115);
        } else {
            if ($security !== $profile->getPasswordRecovery()){
                return array("ERROR"=>116);
            } else {
                if ($profile->getPasswordRecovery() == "" || $profile->getPasswordRecoveryTimer() == ""){
                ...
                if ($timeDifference >3600){
                    ...
                    $profile->resetPasswordRecovery();
                    $profile->uploadProfile();
                    return array("ALERT" => 9,"DATA"=>"Your password has been updated!");
                }
            }
        }
    }
