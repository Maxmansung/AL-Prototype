...
class profileModel extends profile
{
    ...
    public static function insertProfile($profileController, $type){
        ...
        $db = db_conx::getInstance();
        if ($type == "Insert") {
            $req = $db->prepare("INSERT INTO Profile (id, password, profilepicture, email, login, ip, AccountType, gamestatus, avatar, achievements, achievementsSolo, bio, country, gender, age, playStatistics, uploadSecurity, passwordRecovery, passwordRecoveryTimer, cookieKey, shrineScore, forumPosts, reportTimer) VALUES (:profileID, :password2, :profilePicture, :email, :lastLogin, :loginIP, :accountType, :gameStatus, :avatar, :achievements, :achievementsSolo, :bio, :country, :gender, :age, :playStatistics, :uploadSecurity, :passwordRecovery, :passwordRecoveryTimer, :cookieKey, :shrineScore,:forumPosts, :reportTimer)");
        } elseif ($type == "Update"){
            $req = $db->prepare("UPDATE Profile SET password= :password2, profilepicture= :profilePicture, email= :email, login= :lastLogin, ip= :loginIP, AccountType= :accountType, gamestatus= :gameStatus, avatar= :avatar, achievements= :achievements, achievementsSolo= :achievementsSolo, bio= :bio, country= :country, gender= :gender, age= :age, playStatistics= :playStatistics, uploadSecurity= :uploadSecurity, passwordRecovery= :passwordRecovery, passwordRecoveryTimer= :passwordRecoveryTimer, cookieKey= :cookieKey, shrineScore= :shrineScore,forumPosts= :forumPosts, reportTimer= :reportTimer WHERE id= :profileID");
        }
        $req->bindParam(':profileID', $profileID);
