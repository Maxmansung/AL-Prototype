...
function createBackpack(items,maxsize){
    ...
    var counter = 0;
    for (var x in items){
        $(".backpackWrapper").append('<div class="itemObject mr-1 mt-1 objectTooltip" data-toggle="popover" data-placement="top" title="'+items[x].identity+'" data-content="'+items[x].description+'" onclick="dropItem(this.id)" id="itembag'+items[x].itemID+'"><img src="/images/gamePage/items/'+items[x].icon+'" class="itemImage"></div>');
        counter++
    }
    ...
    for (x in items){
        ...
        if (!$("#floor"+items[x].itemTemplateID).length) {
            $("#groundWrapper" + items[x].itemType).append('<div class="itemObjectChest mr-1 mt-1 objectTooltip" data-toggle="popover" data-placement="top" title="'+items[x].identity+'" data-content="'+items[x].description+'" id="floor' + items[x].itemTemplateID + '" onclick="pickGroundItem(this.id)">' +
                '<img src="/images/gamePage/items/'+items[x].icon+'" class="itemImage">' +
                        ...
                        if (created === true) {
                            ...
                            for (var y in buildings[x].itemsRequired) {
                                $(".buildingOverview" + buildings[x].buildingTemplateID + " .buildingItemsWrapper").append(
                                    ...
                                    '<img class="buildingItem pr-1" src="/images/gamePage/items/' + buildings[x].itemsRequired[y].icon + '">' +
...
function buildingListItemWrap(data,visible,button,spacing,button2){
    if (data.isBuilt === false) {
        return '<div class="row ' + visible + ' align-items-center singleBuildingWrapper buildingOverview' + data.buildingTemplateID + '"><div class="col-6 col-sm-5 col-md-4 d-flex flex-row justify-content-start align-items-center" >' + spacing + '<div class="d-flex flex-row" data-toggle="popover" data-placement="top" title="' + data.name + '" data-content="' + data.description + '"><img src="/images/gamePage/buildings/' + data.icon + '" class="buildingImage"><div class="pl-2">' + data.name + '</div></div></div><div class="col-2 d-flex d-sm-none"><button class="btn btn-sm btn-danger py-0 px-1" data-toggle="collapse" data-target="#building' + data.buildingTemplateID + '" aria-expanded="false" aria-controls="building' + data.buildingTemplateID + '"><i class="fas fa-caret-square-down"></i></button></div><div class="buildingItemsWrapper d-none d-sm-flex col-12 col-sm-3 col-md-6 flex-row flex-wrap pt-1 pb-0 pl-1 pr-0"></div><div class="col-4 col-md-2 d-flex flex-row justify-content-end"><div class="mr-3"><span>' + data.staminaSpent + '</span><span>/</span><span>' + data.staminaRequired + '</span></div><button class="btn btn-danger ' + button + ' btn-sm py-0 px-1 font-weight-bold" onclick="' + button2 + '" id="buildingBuild' + data.buildingTemplateID + '"><i class="fas fa-plus"></i></button></div><div class="pt-1 pb-0 pl-1 pr-0 collapse" id="building' + data.buildingTemplateID + '"><div class="d-flex flex-row flex-wrap buildingItemsWrapper"</div></div>';
    } else {
        return '<div class="row align-items-center singleBuildingWrapper buildingOverview' + data.buildingTemplateID + '"><div class="col-6 col-sm-5 col-md-4 d-flex flex-row justify-content-start align-items-center greenBackgroundTransparent" >' + spacing + '<div class="d-flex flex-row" data-toggle="popover" data-placement="top" title="' + data.name + '" data-content="' + data.description + '"><img src="/images/gamePage/buildings/' + data.icon + '" class="buildingImage"><div class="pl-2">' + data.name + '</div></div></div><div class="col-6 col-sm-7 col-md-8 d-flex flex-row justify-content-end greenBackgroundTransparent"><div class="px-2 font-weight-bold">BUILT</div></div></div>';
    }
}
...
function createBackpackUpgradeSingle(data,items){
    var upgradeItems = '<div class="itemObjectChest mr-1 mt-1"><img src="/images/gamePage/items/'+data.cost1Item.icon+'" class="itemImage"><span class="font-size-1 itemObjectChestNumber font-weight-bold" id="1">'+data.cost1Count+'</span></div>';
    if (items === 2){
        upgradeItems += '<div class="itemObjectChest mr-1 mt-1"><img src="/images/gamePage/items/'+data.cost2Item.icon+'" class="itemImage"><span class="font-size-1 itemObjectChestNumber font-weight-bold" id="1">'+data.cost2Count+'</span></div>';
    }
    ...
    if (count < 1) {
        ...
        for (var x in data) {
            if (data[x].usable > 0) {
                ...
                $(".zoneAvatarUseItems").append('<div class="d-flex flex-row clickableFlash"  id="useClick'+data[x].itemID+'" onclick="useItem(this.id)"><img src="/images/gamePage/items/' + data[x].icon + '" class="itemImage"><div class="pl-2 font-size-2">' + action + ' ' + data[x].identity + '</div></div>')
            }
        }
    }
}
...
function createAvatarRecipes(data){
    ...
    if (count > 0){
        for (var x in data){
            $(".zoneAvatarMakeItems").append('<div class="d-flex flex-row clickableFlash" id="recipeClick'+data[x].recipeID+'" onclick="useRecipe(this.id)"><img src="/images/gamePage/recipes/' + data[x].recipeImage + '" class="itemImage"><div class=" pl-2 font-size-2">' + data[x].description + '</div></div>')
        }
    } else {
...
function createAvatarResearch(current,maximum,options){
    ...
    if (current < maximum){
        ...
        for (var x in options){
            ...
            console.log(image);
            $(".zoneAvatarResearchOptions").append('<div class="clickableTransparent researchUpgradeImage" data-toggle="popover" data-placement="top" title="'+options[x].typeName+'" data-content="'+options[x].typeDescription+'" id="researchType'+options[x].typeID+'" onclick="selectResearch(this.id)"><img src="/images/gamePage/buildings/'+image+'" class="itemImage mr-2"></div>');
        }
    }
}
    ...
    if (count > 0 ){
        for (var x in data){
            $(".zoneOtherAvatarResearch").append('<div class="my-1 row justify-content-center align-items-center clickable teachResearchWrap p-1" onclick="teachPlayerResearch(this.id)" id="teach'+avatarID+'+?!'+data[x].buildingTemplateID+'">' +
                '<div class="col-4"><img src="/images/gamePage/buildings/'+data[x].icon+'" class="itemImage"></div>' +
...
function changeView(type){
    switch (type){
        case 1:
            ...
            break;
        case 2:
            $(".showZoneView").removeClass("d-none");
            $(".showAvatarOther").addClass("d-none");
...
function dropItem(id){
    ...
    if (dropLocation === 0) {
        ajax_All(21, 14, typeBuild, newID)
