...
function getItemTestView(){
    ajax_All(229,19,testViewMod)
}

function changeModifier(){
    testViewMod = parseInt($("#modNumber").val());
    getItemTestView();
}
...
function createTestPage(data){
    ...
    for (var x in data){
        $(".testMainWrapper").append('<div class="col-11">' +
            ...
            '<div class="col-3 d-flex flex-column align-items-center justify-content-center" align="center">' +
            ...
            '</div>' +
            '</div>');
        for (var y in data[x].itemArray){
            $("#test"+x).append('<div class="itemTestWrapper mx-md-4 mx-sm-3 mx-2 d-flex flex-column justify-content-center align-items-center">' +
                '<img src="/images/gamePage/items/'+y+'.png" class="itemTestImage">' +
                '<div class="font-size-2">'+data[x].itemArray[y]+'%</div>' +
                '</div>')
        }
    }
}