...
class chatlogAllController
{
    ...
    public static function getAllLogs($avatarID,$day){
        ...
        if ($day === "current" || $day == $map->getCurrentDay()){
            ...
            $scores = shrineController::getMapScores($map->getMapID(),false);
        } else {
            $nightTemperature = $map->getSingleTemperatureRecord($day);
            $personalTemperature = $avatar->getSingleAvatarTempRecord($day);
            $scores = shrineController::getOldMapScores($map->getMapID(),$day);
        }
        ...
        return array("logs" => $totalArray,"day"=>$day,"personalTemp"=>$personalTemperature,"nightTemp"=>$nightTemperature,"currentDay"=>$map->getCurrentDay(),"shrines"=>$scores);
    }
