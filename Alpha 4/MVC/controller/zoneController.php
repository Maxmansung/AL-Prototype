...
class zoneController extends zone
{
    ...
    public function newZone($mapID, $name, $x, $y,$forestLoc,$shrineLoc){
        ...
        $this->biomeType = $this->createBiomeType($forestLoc,$shrineLoc);
    ...
    private function createBiomeType($forestLoc,$shrineLoc)
    {
        $biome = self::biomeForest($forestLoc,$shrineLoc);
        return $this->getBiomeID($biome);
    }

    //This creates a forest on the map
    private function biomeForest($forestLoc,$shrineLoc){
        $xLoc = $forestLoc[0];
        $yLoc = $forestLoc[1];
        $size = rand(2,3);
        $size2 = rand(3,4);
