<?php
include_once($_SERVER['DOCUMENT_ROOT']."/MVC/controller/profile.php");
...
include_once($_SERVER['DOCUMENT_ROOT']."/MVC/model/profileModel.php");
...
class profileController extends profile
{
    ...
    public static function destroysession()
    {
        ...
        if (isset($_COOKIE["loginKey"])) {
            setcookie("loginKey", '', strtotime('-5 days'), '/',".arctic-lands.com");
        }
        ...
        if ($u == "" || $p == "") {
            ...
            if (!password_verify($p,$checkprofile->getPassword())) {
                ...
                if ($cookies == true){
                    ...
                    $cookieUpload .= ':' . $mac;
                    setcookie('loginKey', $cookieUpload, time() + (86400 * 30),"/",".arctic-lands.com");
                    $checkprofile->uploadProfile();
                    return array("COOKIES"=>true);
                } else {
                    $checkprofile->uploadProfile();
                    return array("COOKIES"=>false);
                }
            }
        }
    }
    ...
    public function signup($username, $email, $p,$security){
        ...
        if($u == "" || $e == "" || $p == "" || $sec == ""){
            ...
            $to = "$e";
            $from = "admin@arctic-lands.com";
            $subject = 'Arctic-Lands Account Activation';
            $message = emails::confirmEmail($u,$e,$this->getPassword());
            $headers = "MIME-Version: 1.0\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8\r\n";
            $headers .= "From: inscription@arctic-lands.com\r\n";
        ...
        if ($profile->getProfileID() == ""){
            ...
            if ($profile->getEmail() != $email){
                ...
                if ($checkProfile->getPasswordRecovery() !== $profile->getPasswordRecovery()) {
                    ...
                    $to = "$e";
                    $from = "admin@arctic-lands.com";
                    $subject = 'Arctic-Lands Password Recovery';
                    $message = emails::recoveryEmail($profile->getProfileID(),$profile->getPasswordRecovery());
                    $headers = "MIME-Version: 1.0\r\n";
                    $headers .= "Content-type:text/html;charset=UTF-8\r\n";
                    $headers .= "From: inscription@arctic-lands.com\r\n";
                    $headers .= "Reply-To: ". $from . "\r\n";
