...
function updateStorageOverall(response){
    if (response.storage1.zoneID != null){
        updateStorage(response.storageItems, response.storage1.maximumCapacity);
        storageDetails(response.storage1, response.upgrade, response.lock);
        updateBackpackStorage(response.backpack, response.avatar.maxInventorySlots,true);
    } else {
        updateBackpackStorage(response.backpack, response.avatar.maxInventorySlots,false);
        disableStorage();
    }
...
function disableStorage(){
    ...
    $("#storageWrap").hide();
    $("#storageLockedWriting").hide();
    $("#storageLock").hide();
    $("#storageLevel").empty().append("Not built");

}


function updateBackpackStorage(items,maxBackpack,canDrop){
    ...
    for (var object in items){
        if (canDrop === true) {
            ...
            $("#backpackStorage").append("<div class='imagediv'><image class='itemimage' src='/images/items/" + items[object].icon + "' id='" + items[object].itemID + "'><span class='imagetext'>" + items[object].identity + "<hr>" + items[object].description + "</span></div>")
        }
    }
...
function storageDetails(storage,upgrade,lock){
    ...
    if (lock.total !== false){
        ...
        $("#storageLock").empty()
            .append("<div>The chest lock has<strong>&nbsp"+lock.current+"/"+lock.total+" armour&nbsp</strong>left</div>");
        if (lock.access === true){
