...
function updateMapImage(response){
    ...
    playerVision(response.itemsView.avatar);
    zoneInformation(response.zone);
    $("#infobox").hide();
    if (response.zone.isSpecialZone !== false){
        $("#grounditemswrap").hide();
        $("#destroyBiome").hide();
        $("#playerActionsWrap").hide();
    } else {
        $("#grounditemswrap").show();
        $("#destroyBiome").show();
        $("#playerActionsWrap").show();
...
function playerVision(avatar) {
    $("#"+avatar.zoneID).append("<img src='../images/playerlocation.png' id='playerimg' class='mapimages'>");
    var playerLoc = parseInt(avatar.zoneID.slice(9,13));
...
function updateItems(info){
    updateBackpack(info.backpack, info.avatar.maxInventorySlots);
    updateGround(info.ground);
    updateRecipes(info.recipes);
