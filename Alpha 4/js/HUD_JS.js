...
function updateTemperature(response){
    ...
    $("#tempdatawrite").empty()
        .append("<div class='temperatureSpan'><span id='tempwrite'>Night Temp: </span><br>"+(parseInt(response.nightTemp)*-1)+"&degC</div><div class='moreInfoButton'><img src='/images/downArrowMaps.png'> <div id='tempOverviewHidden'>"+moreDetail+"</div></div><div class='temperatureSpan'><span id='tempwrite'>Survivable Temp: </span><br>"+(parseInt(response.calcSurvTemp)*-1)+"&degC</div>")
}
...
function writeTempBonuses(response){
    ...
    if (response.tempBuildings-response.tempFirepit !== 0) {
        returnText += "<div class='tempDetailItem'>Other Buildings: " + response.tempBuildings-response.tempFirepit + "</div>";
    }
...
function updateReadiness(status){
    if (status == true){
        ...
        $("#readywrap").empty()
            .append( "<img id='readyimage' src='/images/buttonready3.png' onclick='setReady()'>");
    }
}
